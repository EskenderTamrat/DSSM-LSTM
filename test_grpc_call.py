...
class TestSuiteGrpc(unittest.TestCase):
	def setUp(self):
		...
		self.server.start()
		self.result = ('qry_ans_similarity: 0.9997633099555969\n'
			'qry_poor_ans_similarity: 0.9997578263282776\n')

	def test_grpc_call(self):
		with grpc.insecure_channel('localhost:' + self.port) as channel:
			stub = DSSMStub(channel)
			request = DSSMRequest(qry="what contribution did you made to science", ans1 ="book author book_editions_published", ans2="language human_language main_country")
