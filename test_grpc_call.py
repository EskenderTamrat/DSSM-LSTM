...
class TestSuiteGrpc(unittest.TestCase):
	def setUp(self):
		...
		self.server.start()
		self.result = ('qry_ans_similarity: 0.99989253282547\n'
	...
	def test_grpc_call(self):
		with grpc.insecure_channel('localhost:' + self.port) as channel:
			...
			request = DSSMRequest(qry="what contribution did you made to science", 
				ans1 ="book author book_editions_published", ans2="activism address adjoining_relationship")
